function [X, Ac, Sw, n, nsw] = PT_AMC_all(K, T, N, C, lambdas, Lambda, rho, alpha, theta, p, init, npairs, deltat, correct)
...
parfor c=1:C
    ...
    for t = tis
        ...
        while ti <= t
            ...
            %at the same time as the deadline t
            if(~update_first)
                % hold k-th state
                if(h(k)<=1)
            ...
            if (ti <= t) %do not update if deadline has occured
                ...
                can = normrnd(x(k), rho);
                % odds ratio
                A1 = gampdf_mixture_temp(can, lambdas1(k), Lambda, alpha, theta);
                A2 = gampdf_mixture_temp(x(k), lambdas1(k), Lambda, alpha, theta);
                A = A1/A2;
