function [X, n, nsw] = PT_AMC_nocold(K, T, N, C, lambdas, Lambda, rho, alpha, theta, p, init, npairs, deltat, correct)
% Single processor Anytime Parallel Tempeting Monte Carlo algorithm
% Notes:    No local moves on the cold chain
%           Only outputs the cold chain
% Inputs:   K: # chains
        ...
        while ti <= t
            ...
            if (ti <= t) %do not update if deadline has occured
                if(k ~= 1) % do not update the cold chain                    
                    ...
                    can = normrnd(x(k), rho);
                    % odds ratio
                    A1 = gampdf_mixture_temp(can, lambdas(k), Lambda, alpha, theta, 0.5);
                    A2 = gampdf_mixture_temp(x(k), lambdas(k), Lambda, alpha, theta, 0.5);
                    A = A1/A2;
