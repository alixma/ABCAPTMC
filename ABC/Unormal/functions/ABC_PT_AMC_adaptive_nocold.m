function [Theta, X, n, ne, nsw, sw] = ABC_PT_AMC_adaptive_nocold(K, Tt, N, C, y, sigma, epsilon, prior_params, rho, deltat, pre, correct)
...
parfor c=1:C
    ...
    while t<=T
        ...
        while ti<=deltat
            %% LOCAL MOVES %%
            if(k>1) % do not update cold chain
                if(~resume)
