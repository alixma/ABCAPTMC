function [Theta, X, Rej, n, ne, nsw, sw, TM] = MA_ABC_standard_exchange(C,  params, simulations)
% Standard ABC algorithm on a MA(q) example

params1 = params; params1.deltat = params.T+1000000;
...
while t<=params.T
    ...
    while(ti<params.deltat) && (t<=params.T)
        theta_in = theta1(:,k);
        x_in = x1(:,k);
        [theta_in, x_in, resume, ~, rej] = MA_local_moves(r, all, ti, theta_in, x_in, params1, params.rho(k), simulations, eps1(:,k), resume);
        ti = ti+(rej~=2);
