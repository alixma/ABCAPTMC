function multi_timeplots_twice_old(TM_in, titl, lim, names, standard, col, edgecol)
...
if~exist('names', 'var')
    names= {'Within workers', 'Exchange between workers', 'Worker 1: local', 'Worker 1: exchange', 'Worker 2: local', 'Worker 2: exchange'};
end

if~exist('col', 'var')
    col = {rgb('DarkOrange'), rgb('Coral'), rgb('SandyBrown'), rgb('Orange'), rgb('SandyBrown'), rgb('Orange'),...
        rgb('SandyBrown'), rgb('Orange'),rgb('SandyBrown'), rgb('Orange')};
end
if~exist('edgecol', 'var')
    edgecol= {rgb('Red'), rgb('DarkOrange'), rgb('DarkOrange'), rgb('Orange'), rgb('DarkOrange'), rgb('Orange'),...
        rgb('DarkOrange'), rgb('Orange'), rgb('DarkOrange'), rgb('Orange')};
end
...
workers = [combined_exchange; worker_starttimes]; workers = workers(:)';
...
% exchange moves
startTimes_a = [{exchange_a(1:(end-1)), local_a}, workers];
% add in start times for full set of parallel updates and intra worker
% exchange moves
endTimes_a = [{local_a, exchange_a(2:end)}, workere];%{local_a, exchange_a, local_w(:,1), local_w(:,2)};
