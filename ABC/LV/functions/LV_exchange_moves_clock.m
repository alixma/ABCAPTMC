function [theta_out, x_out, rej_out, n_out, swap]=LV_exchange_moves_clock(deadline, theta_in, x_in, n_in, observations, LV, exchange, epsilon_in, swap)
% Anytime exchange ABC moves for LV model
...
deadline.epsilon = epsilon_in(swap(2)); % for early interruption of simulation
% 
% % sample until larger ball is hit
while(sample)
    [x_swap, rej_swap] = get_x_clock(LV, observations, theta_in(swap(2),:), deadline);
    acc_y = abs(log(x_swap)-log(observations.y));

    if(sum(acc_y<epsilon_in(swap(2)))==nET)||(rej_swap==-1)
        mkswap=(sum(acc_y<epsilon_in(swap(1)))==nET);
        break
    end
end
% fast exchanges
