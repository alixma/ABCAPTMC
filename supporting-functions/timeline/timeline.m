...
else %%/ ADDED 19/06/2018
    % Patch edge colour specified; extract in a variable and remove from
    % list of arguments to patch().
    for iField=1:length(fieldNames)
        thisFieldName = fieldNames{iField};
        if strcmpi(thisFieldName,'edgecolor')
            edgeColor = patchArgsStruct.(thisFieldName);
...
for iLine = 1:numLines
    ...
    numPatches = min(length(thisLineStartTimes), length(thisLineEndTimes));
    ...
    for iPatch = 1:numPatches
        ...
        thisLinePatchHndls(iPatch) = patch(thisPatchX,thisPatchY,faceColor{iLine},'edgecolor', edgeColor{iLine}, patchArgs{:});
    end % for each patch
